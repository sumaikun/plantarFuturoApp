...
import { createForestUnitPhase2 ,
   ...
   addToBatchData } from '../flux/actions';
...
class FormProcess extends Component {
  ...
  componentDidMount(){
    ...
    if(!window.navigator.onLine)
    {
      ...
      console.log("Modo offline form proccess");
  ...
  submitData(e){
    ...
    {
      console.log("Agregando en modo offline");
      let data = this.state.formData;
      data.functional_unit_id = this.props.appState.currentFunctionalUnit.id;
      data.user_id = this.props.appState.user.id;
      data.metaDesc = "ProccessFormBatchData";
      this.props.addToBatchData(data);

      Ons.notification.alert({title:"",message:" Dato registrado en memoria "});
      ...
      <div style={{backgroundColor:"#e6e7e8",height:"100%"}}>
        ...
        <form className="simpleForm"  onSubmit={this.submitData}>
          ...
          <Row>
            ...
            <Col>
              <Card style={styles.cardInput}>
                <Input style={styles.textInput} name="code" value={this.state.formData.code} onChange={this.handleChangeInput} placeholder="Codigo" required />
              </Card>
            </Col>
...
export default  connect(mapStateToProps, { createForestUnitPhase2 ,  updateForestUnitPhase2 ,
   getForestalUnits, addToBatchData })(FormProcess);
