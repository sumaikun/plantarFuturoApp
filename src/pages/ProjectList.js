...
//Onsen Ui
import {  Col, Row, Card, Button, List, ListItem, BottomToolbar} from 'react-onsenui';
...
resetFunctionalUnits, selectProject , goToRiskManagement ,
goToProcessForm } from '../flux/actions';
...
class ProjectList extends Component {
  ...
  componentDidMount(){
    ...
    //console.log("Fase del proyecto "+this.props.appState.currentPhase);
    if(!window.navigator.onLine && this.props.appState.currentPhase == 2)
    {
      this.setState({enableOfflineregister:true});
    }
  ...
  registerUnit(e){
    ...
    if(!this.props.appState.isFetching)
    {
        ...
        let componentSuccess = (reloadProjectId) => {
          this.Code.value = "";
          let modal = document.querySelector('#newFunctionalUnitModal');
  ...
  render() {
    ...
    return (
      <AppPage  title={[<strong>{titlePage}</strong>]} backButton={true} backButtonCallBack={()=>{ this.props.fetchProjects() }}>
          ...
          currentPhaseProjects.map((project, i) => {
              ...
              <div onClick={()=>{ this.props.appState.currentPhase == "4" ? (()=>{
                ...
                <ListAccordion counter={i+1} projectName={project.name}  phase={this.props.appState.currentPhase}  projectInfo={project.customer.name} >
                  ...
                  {
                    this.state.enableOfflineregister ?
                   <BottomToolbar modifier="material">
                    <div style={styles.offLineLabel} onClick={()=>{this.props.goToProcessForm() }}>
                      <span>Agregar registro offline </span>
                      <i style={ { marginLeft: "5%" } } class="fas fa-unlink"></i>
                    </div>
                   </BottomToolbar>:null
                  }
        ...
        <Modal ModalId="newFunctionalUnitModal" title="Registro de información" ModalStyles={styles.modalStyles}>
...
export default  connect(mapStateToProps, { goToForestalUnits , createFunctionalUnit,
  getFunctionalUnits, updateFunctionalUnit, getForestalUnits, setFunctionalUnit,
  fetchProjects, resetFunctionalUnits, selectProject , goToRiskManagement,
  goToProcessForm })(ProjectList);
